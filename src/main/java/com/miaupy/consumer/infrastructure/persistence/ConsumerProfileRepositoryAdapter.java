...
class ConsumerProfileRepositoryAdapter implements ConsumerProfileRepository {
    ...
    ConsumerProfileRepositoryAdapter(SpringDataConsumerProfileRepository repository) { this.repository = repository; }

    public ConsumerProfile save(ConsumerProfile p) { return map(repository.save(map(p))); }
    ...
    public Optional<ConsumerProfile> findById(UUID id) { return repository.findById(id).filter(e -> e.active).map(this::map); }

    private ConsumerProfileJpaEntity map(ConsumerProfile p) {
        return new ConsumerProfileJpaEntity(p.id(), p.authSubject(), p.name(), p.email(), p.phone(), p.document(),
                p.birthDate(), p.active(), p.createdAt(), p.updatedAt(), p.version());
    }
    private ConsumerProfile map(ConsumerProfileJpaEntity e) {
        return new ConsumerProfile(e.id, e.authSubject, e.name, e.email, e.phone, e.document, e.birthDate, e.active,
